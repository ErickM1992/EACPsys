...
ElseIf Vendedores.NoMatch Then
    MsgBox "Por favor especifique un vendedor valido", vbCritical
...
If KeyAscii = 13 And Text3.Text <> "" Then
    If IsNumeric(Text3.Text) = True Then
        If 0 < CDbl(Text3.Text) And CDbl(Text3.Text) <= CDbl(Label10.Caption) Then
            Clientes.Index = "PrimaryKey"
            Clientes.Seek "=", Combo1
            If Not Clientes.NoMatch Then
                Call Calcular
            Else
                MsgBox "Para continuar con esta operación, debe seleccionar un cliente válido", vbExclamation
            End If
        Else
